...
class MainApp(App):
    ...
    def carregar_infos_usuario(self):
        try:
            ...
            # pegar informaçãoes do usuário
            requisicao = requests.get(
                f"https://aplicativovendashash-39c91-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}")
    ...
    def mudar_foto_perfil(self, foto, *args):
        ...
        # macete para passar a informação foto que deve ir como dicionario {"avatar":foto}
        info = f'{{"avatar": "{foto}"}}'
        requisicao = requests.patch(f"https://aplicativovendashash-39c91-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
                                    data=info)
        self.mudar_tela('ajustespage')

    def adicionar_vendedor(self, id_vendedor_adicionado):
        link = f'https://aplicativovendashash-39c91-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor_adicionado}"'
        ...
        if requisicao_dic == {}:
            ...
            if id_vendedor_adicionado in equipe:
                mensagem_texto.text = "Vendedor já está na equiope !!"
            else:
                self.equipe = self.equipe +f",{id_vendedor_adicionado}"
                info = f'{{"equipe": "{self.equipe}"}}'
                requests.patch (f"https://aplicativovendashash-39c91-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
                               data=info)
    ...
    def selecionar_cliente(self, foto, *args):
        ...
        # pintar de branco todos os outros
        # foto = Nome.png !!!
            try:
                texto = item.text
                texto = texto.lower() + ".png"
                if foto == texto:
                    item.color = (0, 207/255, 21/255, 1)
            except:
                pass
    ...
    def selecionar_unidade(self, id_label, *args):
        ...
        # pintar o selecionado
        pagina_adicionarvendas.ids[id_label].color = (0, 207/255, 21/255, 1)
    ...
    def adicionar_venda(self):
        ...
        quantidade = pagina_adicionarvendas.ids['quantidade'].text
            ...
            # pinta de vermelho se não for preenchido
        if not cliente:
        ...
        # dado que preencheu tudo adicionar vendas
        if cliente and produto and unidade and preco and quantidade and \
            ...
            requests.post(f"https://aplicativovendashash-39c91-default-rtdb.firebaseio.com/{self.local_id}/vendas.json?auth={self.id_token}",
                          data=info)
            ...
            requisicao = requests.get(f"https://aplicativovendashash-39c91-default-rtdb.firebaseio.com/{self.local_id}/total_vendas.json?auth={self.id_token}")
    ...
    def carregar_todas_vendas(self):
        ...
        # pegar informaçãoes da empresa
        requisicao = requests.get(
            f'https://aplicativovendashash-39c91-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"')
        requisicao_dic = requisicao.json()
    ...
    def carregar_vendas_vendedor(self, dic_info_vendedor, *args):
        ...
        pagina_vendasoutrovendedor.ids["label_total_vendas"].text = \
            f"[color=#000000]Total de Vendas:[/color][b] R$ {total_vendas} [/b]"
